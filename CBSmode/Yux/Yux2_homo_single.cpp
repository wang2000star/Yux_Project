          ...
          TFHEpp::EvalKey &ek)
{
    ...
    std::vector<TRLWE_1> lut_result0(4);
    std::vector<std::vector<TLWE_1>> Sbox_value(16, std::vector<TLWE_1>(8));
    int index[16] = {1, 2, 3, 0, 5, 6, 7, 4, 9, 10, 11, 8, 13, 14, 15, 12};
    ...
    for (int r = 0; r < 4; r++)
    {
        ...
        tfhe_mul(lut_result0[1], Table1, Table2, Table3, Table4, bootedTRGSW[5], bootedTRGSW[6]);
        tfhe_mul(lut_result0[2], Table1, Table2, Table3, Table4, bootedTRGSW[9], bootedTRGSW[10]);
        tfhe_mul(lut_result0[3], Table1, Table2, Table3, Table4, bootedTRGSW[13], bootedTRGSW[14]);

        TRLWE1_XOR_Four(lut_result[3], lut_result[3], lut_result0[0], lut_result[2], RoundConstantTRLWE);
        TRLWE1_XOR_Four(lut_result[7], lut_result[7], lut_result0[1], lut_result[6], RoundConstantTRLWE);
        TRLWE1_XOR_Four(lut_result[11], lut_result[11], lut_result0[2], lut_result[10], RoundConstantTRLWE);
        TRLWE1_XOR_Four(lut_result[15], lut_result[15], lut_result0[3], lut_result[14], RoundConstantTRLWE);
...
#endif
    std::cout << "Yux密钥扩展的tlwe加密完成" << endl;
...
// 最后一轮第12轮解密
    ...
    if (flag)
    {
        std::cout << "第" << ROUND << "轮XOR解密成功!!!" << endl;
    }
    else
    {
        std::cout << "第" << ROUND << "轮XOR解密失败" << endl;
        return 0;
    }
    // 记录解密验证时间
    ...
    if (flag)
    {
        std::cout << "第" << ROUND << "轮Sbox解密成功!!!" << endl;
    }
    else
    {
        std::cout << "第" << ROUND << "轮Sbox解密失败" << endl;
        // return 0;
    }
    // 记录解密验证时间
    ...
    std::cout << "第" << ROUND << "轮XOR用时：" << elapsed_seconds_XOR.count() << "s" << endl;
    std::cout << "第" << ROUND << "轮Sbox用时：" << elapsed_seconds_Sbox.count() << "s" << endl;
    ...
    for (int r = ROUND - 1; r > 0; r--) // 11 rounds
    {
        ...
        if (flag)
        {
            std::cout << "第" << r << "轮XOR解密成功!!!!!!" << endl;
        }
        else
        {
            std::cout << "第" << r << "轮XOR解密失败" << endl;
            // return 0;
        }
        // 记录解密验证时间
        ...
        if (flag)
        {
            std::cout << "第" << r << "轮线性层解密成功!!!!!!" << endl;
        }
        else
        {
            std::cout << "第" << r << "轮线性层解密失败" << endl;
            // return 0;
        }
        // 记录解密验证时间
        ...
        if (flag)
        {
            std::cout << "第" << r << "轮Sbox解密成功!!!" << endl;
        }
        else
        {
            std::cout << "第" << r << "轮Sbox解密失败" << endl;
            // return 0;
        }
        // 记录解密验证时间也是结束时间
        ...
        std::cout << "第" << r << "轮解密完成" << endl;
        std::cout << "第" << r << "轮XOR用时：" << elapsed_seconds_XOR.count() << "s" << endl;
        std::cout << "第" << r << "轮LinearBootstrap用时：" << elapsed_seconds_LinearBootstrap.count() << "s" << endl;
        std::cout << "第" << r << "轮Linear用时：" << elapsed_seconds_Linear.count() << "s" << endl;
        std::cout << "第" << r << "轮Sbox用时：" << elapsed_seconds_Sbox.count() << "s" << endl; 
        std::cout << "第" << r << "轮解密用时：" << elapsed_seconds.count() << "s" << endl;  
        total_time += elapsed_seconds.count();
        total_time_XOR += elapsed_seconds_XOR.count();
...
// 白化轮即第0轮解密
    ...
    if (flag)
    {
        std::cout << "第0轮XOR解密成功!!!" << endl;
    }
    else
    {
        std::cout << "第0轮XOR解密失败" << endl;
    }
    // 记录解密验证时间也是结束时间
    end1 = std::chrono::system_clock::now();
    elapsed_seconds = end1 - start - (end1 - start1);
    std::cout << "第0轮解密完成" << endl;
    std::cout << "第0轮解密(XOR)用时：" << elapsed_seconds.count() << "s" << endl;
    total_time += elapsed_seconds.count();
