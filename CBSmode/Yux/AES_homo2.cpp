...
// AES密钥扩展
extern long AESKeyExpansion(unsigned char roundKeySchedule[], unsigned char key[], int keyBits);
// AES白化轮
extern void AES_WhiteRound(const std::array<unsigned char, 16> &plain, std::array<unsigned char, 16> &state, const std::array<unsigned char, 16> &round_key);
// AES常规轮
extern void AES_Round(std::array<unsigned char, 16> &y, const std::array<unsigned char, 16> &key);
// AES最后一轮
extern void AES_LastRound(std::array<unsigned char, 16> &y, const std::array<unsigned char, 16> &key);
