...
void CipherShiftRowsInverse(std::vector<std::vector<TLWE_0>> &cipher)
{
    ...
    for (int j = 0; j < 8; j++)
    {
        for (int i = 0; i < 16; i++)
        {
            ...
            {
                // 第 0 行保持不变
                TFHEpp::HomCOPY<typename bkP::domainP>(cipher[i][j], temp[i][j]);
            }
            else if (i % 4 == 1)
            {
                // 第 1 行向右移 1 位
                TFHEpp::HomCOPY<typename bkP::domainP>(cipher[i][j], temp[(i + 3) % 16][j]);
            }
            else if (i % 4 == 2)
            {
                // 第 2 行向右移 2 位
                TFHEpp::HomCOPY<typename bkP::domainP>(cipher[i][j], temp[(i + 2) % 16][j]);
            }
            else if (i % 4 == 3)
            {
                // 第 3 行向右移 3 位
                TFHEpp::HomCOPY<typename bkP::domainP>(cipher[i][j], temp[(i + 1) % 16][j]);
            }
        }
    }
}
...
// 用于解密的 CipherMul0e 操作
void CipherMul0e(std::vector<TLWE_0> &byte, std::vector<TLWE_0> &consByte)
{
    // 模拟乘法 0x0e 的过程，相当于多次 xtime 操作和 XOR 的组合
    std::vector<TLWE_0> tmp2(8);
    std::vector<TLWE_0> tmp4(8);
    std::vector<TLWE_0> tmp8(8);

    CipherMul2(byte, consByte); // 乘以 2
    for (int i = 0; i < 8; i++)
    {
        TFHEpp::HomCOPY<typename bkP::domainP>(tmp2[i], byte[i]);
    }

    CipherMul2(byte, consByte); // 再次乘以 2
    for (int i = 0; i < 8; i++)
    {
        TFHEpp::HomCOPY<typename bkP::domainP>(tmp4[i], byte[i]);
    }

    CipherMul2(byte, consByte); // 再次乘以 2
    for (int i = 0; i < 8; i++)
    {
        TFHEpp::HomCOPY<typename bkP::domainP>(tmp8[i], byte[i]);
    }

    // XOR 混合 0x0e = 0x08 ^ 0x04 ^ 0x02
    XOR_Two(byte, tmp8, tmp4, 8);
    XOR_Two(byte, byte, tmp2, 8);
}

// 同样定义 CipherMul0b, CipherMul0d, CipherMul09
void CipherMul0b(std::vector<TLWE_0> &byte, std::vector<TLWE_0> &consByte)
{
    std::vector<TLWE_0> tmp2(8);
    std::vector<TLWE_0> tmp4(8);
    std::vector<TLWE_0> tmp8(8);
    ...
    // XOR 混合 0x0b = 0x08 ^ 0x02 ^ 0x01
    XOR_Two(byte, tmp8, tmp2, 8);
    XOR_Two(byte, byte, consByte, 8); // consByte 是原来的 byte
}

void CipherMul0d(std::vector<TLWE_0> &byte, std::vector<TLWE_0> &consByte)
{
    std::vector<TLWE_0> tmp2(8);
    std::vector<TLWE_0> tmp4(8);
    std::vector<TLWE_0> tmp8(8);
    ...
    // XOR 混合 0x0d = 0x08 ^ 0x04 ^ 0x01
    XOR_Two(byte, tmp8, tmp4, 8);
    XOR_Two(byte, byte, consByte, 8);
}
void CipherMul09(std::vector<TLWE_0> &byte, std::vector<TLWE_0> &consByte)
{
    std::vector<TLWE_0> tmp2(8);
    std::vector<TLWE_0> tmp8(8);
    ...
    // XOR 混合 0x09 = 0x08 ^ 0x01
    XOR_Two(byte, tmp8, consByte, 8);
}
void CipherInvMixColumns(std::vector<std::vector<TLWE_0>> &cipher, std::vector<TLWE_0> &consByte)
{
    for (int i = 0; i < 4; i++)
    {
        ...
        for (int j = 0; j < 8; j++)
        {
            TFHEpp::HomCOPY<typename bkP::domainP>(Tm[j], cipher[4 * i + 0][j]);
        }
        XOR_Two(Tm, Tm, cipher[4 * i + 1], 8);
        CipherMul0e(Tm, consByte);

        XOR_Two(cipher[4 * i + 0], cipher[4 * i + 0], Tm, 8);  // 乘 0x0e
        XOR_Two(cipher[4 * i + 0], cipher[4 * i + 0], Tmp, 8); // 更新列
        ...
        XOR_Two(Tm, Tm, cipher[4 * i + 2], 8);
        CipherMul0b(Tm, consByte);

        XOR_Two(cipher[4 * i + 1], cipher[4 * i + 1], Tm, 8); // 乘 0x0b
        XOR_Two(cipher[4 * i + 1], cipher[4 * i + 1], Tmp, 8);
        ...
        XOR_Two(Tm, Tm, cipher[4 * i + 3], 8);
        CipherMul0d(Tm, consByte);

        XOR_Two(cipher[4 * i + 2], cipher[4 * i + 2], Tm, 8); // 乘 0x0d
        XOR_Two(cipher[4 * i + 2], cipher[4 * i + 2], Tmp, 8);
        ...
        XOR_Two(Tm, Tm, t, 8);
        CipherMul09(Tm, consByte);

        XOR_Two(cipher[4 * i + 3], cipher[4 * i + 3], Tm, 8); // 乘 0x09
        XOR_Two(cipher[4 * i + 3], cipher[4 * i + 3], Tmp, 8);
    }
}
//
template <class bkp>
...
// 显然后面的实现更简单，但是实际应用中如果只发送了初始密钥的加密结果，那么需要对加密后的初始密钥进行扩展，这里的实现就是对加密后的初始密钥进行扩展，则只能用上面的方法
    ...
    {
        int bin_str[8];
        HexToBinStr(RoundKey[i], bin_str, 8); // 将十六进制密钥转换为二进制字符串
        rk[i].resize(8);
        for (int j = 0; j < 8; j++)
        {
            // encrypt TLWE in level 0
...
*/
    ...
    // 按行输出RounKey2D
    // 输出加密前和解密后的轮密钥
    std::cout << "解密后的轮密钥与原始轮密钥进行比较" << std::endl;
    flag = 1;
    for (int i = 0; i < 11; i++)
    {
        for (int j = 0; j < 16; j++)
        {
            ...
            {
                std::cout << "解密后的轮密钥与原始轮密钥不一致" << std::endl;
                // 输出i,j
    ...
    if (flag == 1)
    {
        std::cout << "解密后的轮密钥与原始轮密钥一致" << std::endl;
    }
#endif
// 前面已经得到了AES密文且用tlwe对其进行加密得到了cipher，现在使用tlwe下的AES密钥扩展对cipher进行AES解密，这样就间接得到了用tlwe直接加密明文结果
#if 1
// 第10轮解密
    std::cout << "第10轮tlwe解密开始" << endl;
    // 调用解密函数
    decryptTLWE(cipher, *sk, decrypt);
    std::cout << "第10轮tlwe解密完成" << endl;
    // 检查解密结果是否正确
    flag = check_decrypt(decrypt, state);
    if (flag)
    {
        std::cout << "第10轮tlwe解密成功" << endl;
    }
    else
    {
        std::cout << "第10轮tlwe解密失败" << endl;
        return 0;
    }
    std::vector<std::vector<TFHEpp::TRGSWFFT<typename privksP::targetP>>> bootedTRGSW;
    ...
    // 验证此时是否解密成功
    std::cout << "第0轮tlwe解密开始" << endl;
    // 调用解密函数
    decryptTLWE(cipher, *sk, decrypt);
    std::cout << "第0轮tlwe解密完成" << endl;
    // 检查解密结果是否正确
    flag = check_decrypt(decrypt, AES_state[0]);
    if (flag == 0)
    {
        std::cout << "第0轮tlwe解密失败" << endl;
    }
    else
    {
        std::cout << "第0轮tlwe解密成功" << endl;
    }
    // 白化轮解密
    std::cout << "白化轮解密开始" << endl;
    // 使用切片进行 XOR_Two 操作
    ...
    std::cout << "白化轮解密完成" << endl;
#endif
// 至此，我们得到了用tlwe加密的AES明文的结果，下面将其用tlwe私钥解密，检验解密结果是否正确
#if 1
    std::cout << "tlwe最后解密开始" << std::endl;
    // 调用解密函数
    decryptTLWE(cipher, *sk, decrypt);
    std::cout << "tlwe最后解密完成" << endl;
    // 输出解密结果
    /*std::cout << "tlwe最后解密结果为：" << endl;
    for (int i = 0; i < 16; i++)
    {
    ...
    */
    ...
    if (flag == 0)
    {
        std::cout << "tlwe最后解密失败" << endl;
    }
    else
    {
        std::cout << "tlwe最后解密成功" << endl;
    }
#endif
    return 0;
}
//下一步首先存储AES每一步的密文状态，和同步AES的tlwe解密结果，然后对比两者是否一致