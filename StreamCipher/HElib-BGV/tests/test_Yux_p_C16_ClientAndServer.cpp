...
static const long RoundConstant = 0xCD;
...
// encodeTo16Ctxt对应的解码
void decodeTo16Ctxt(vector<long> &data, const vector<vector<long>> &encData, const EncryptedArray &ea)
{
...
// 函数：解密并验证密文是否正确，需要解码
bool verifyDecryption16(const std::vector<Ctxt> &encryptedVec, const vector<long> &originalVec, const SecKey &secretKey, const EncryptedArray &ea)
{
...
bool verify_encryptSymKey(vector<Ctxt> &encryptedSymKey, const vector<long> &SymKey, const SecKey &secretKey, EncryptedArray &ea)
{
    ...
    bool isDecryptedSymKeyCorrect = std::equal(decryptedSymKey.begin(), decryptedSymKey.end(), SymKey.begin());
    auto end = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double> elapsed_seconds = end - start;
    std::cout << "Decryption and verification finished! Time: " << elapsed_seconds.count() << "s\n";
...
// 线性层HE计算
void HE_Linear(vector<Ctxt> &eData)
{
...
// S盒HE计算
void HE_Sbox(vector<Ctxt> &eData, const ZZX &encA)
{
    ...
    for (int i = 0; i < 16; i += 4)
    {
        for (int j = 0; j < 2; j++)
        {
            ...
            temp2 += c0;
            temp2 += c3;
            temp2.addConstant(encA);

            Ctxt temp3(c2);
            temp3.multiplyBy(c3);
            temp3 += c1;
            temp3 += temp2;
            temp3.addConstant(encA);
...
#pragma omp critical
    ...
    vector<ZZX> encoded_expandedRoundConstant;
    vector<long> expandedRoundConstant(PlainByte, RoundConstant);
    encodeTo16Ctxt(encoded_expandedRoundConstant, expandedRoundConstant, ea); // encode as HE plaintext
    ZZX encoded_RoundConstant = encoded_expandedRoundConstant[0];
