...
int main()
{
    ...
    {     Xset01[i]=Xset[3*i];
          Xset01[i].multiplyBy(Xset[3*i+1]);
          Xset02[i]=Xset[i];
          Xset02[i].multiplyBy(Xset[3*i+2]);
    }
    ...
    //     std::cout << "time1: " << std::chrono::duration<double>(s2 - s1).count() << "s\n";
    // }
    // for (int i = 0; i < len3; i++)
    // {
    ...
    if (Rkflag)
    {
        for (int i = 0; i < eRk_len; i++)
        {
            encryptedRoundKeySet[i].multByConstant(Xset[i]);
        }
    }
    else
    {
        for (int i = 0; i < eRk_len; i++)
        {
            encryptedRoundKeySet[i].addConstant(Xset[i]);
        }
    }
    ...
    helib::Ptxt<helib::BGV> ptxt0(*context);
    ...
    for (long i = 0; i < BlockByte; i += 3) // BlockByte
    {
        ...
        ptxt0 = Xset[i + 1];
        ptxt0 *= IV[i];
        K1Iv0R1.multByConstant(ptxt0);
        K0Iv1R0 = encryptedSymKey[i + 0];
        ptxt0 = Xset[i];
        ptxt0 *= IV[i + 1];
        K0Iv1R0.multByConstant(ptxt0);
        K2Iv0R2 = encryptedSymKey[i + 2];
        ptxt0 = Xset[i + 2];
        ptxt0 *= IV[i];
        K2Iv0R2.multByConstant(ptxt0);
        K0Iv2R0 = encryptedSymKey[i + 0];
        ptxt0 = Xset[i + 2];
        ptxt0 *= IV[i + 2];
        K0Iv2R0.multByConstant(ptxt0);
