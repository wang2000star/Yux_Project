...
void HE_M(std::vector<Ctxt> &encryptedKeyStream, std::vector<vec_long> xof_mat1, std::vector<vec_long> xof_mat2,
          std::vector<vec_long> xof_rc, Cmodulus &cmodulus, int r)
{
    ...
    for (int i = 0; i < BlockPlainWords; i++)
    {
        ...
        cmodulus.iFFT(encodedtemp, xof_mat2[s1 + i * BlockPlainWords + 0]);
        ...
        for (int j = 1; j < BlockPlainWords; j++)
        {
            ...
            cmodulus.iFFT(encodedtemp, xof_mat2[s1 + i * BlockPlainWords + j]);
            convert(encodedxof_mat2, encodedtemp);
            temp1.multByConstant(encodedxof_mat2);
            encryptedKeyStream[i] += temp1;
        }
        cmodulus.iFFT(encodedtemp, xof_rc[s2 + i + BlockPlainWords]);
        convert(encodedxof_rc, encodedtemp);
...
int main()
{
    ...
    for (int test = 0; test < 3; test++)
    {
        ...
        HE_M(encryptedKeyStream, xof_mat1, xof_mat2, xof_rc, cmodulus, 0);
        ...
        for (long r = 1; r < Nr; r++)
        {
            ...
            // Linear Layer
            HE_M(encryptedKeyStream, xof_mat1, xof_mat2, xof_rc, cmodulus, r);
        ...
        start_linear = std::chrono::high_resolution_clock::now();
        // Linear Layer
        HE_M(encryptedKeyStream, xof_mat1, xof_mat2, xof_rc, cmodulus, Nr);
        end_linear = std::chrono::high_resolution_clock::now();
        Linear_time += std::chrono::duration<double>(end_linear - start_linear).count();
