    ...
    for (long i = 0; i < BlockByte; i++)
    {
        ...
        for (long r = 0; r < R; r++)
        {
            for (long j = 0; j < PlainBlock; j++)
            {
                ...
                // if (byteIdx < data_size)
                // {
                //     slots[j] = data[byteIdx];
                // }
                // assert(byteIdx < data_size && "byteIdx 超出 data_size 范围");
                slots[j] = data[byteIdx];
            }
...
void HE_MC_MR_2(vector<Ctxt> &eData)
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        int id0 = index[0] + s, id1 = index[1] + s, id2 = index[2] + s, id3 = index[3] + s, 
            ...
            id8 = index[8] + s, id9 = index[9] + s, id10 = index[10] + s, id11 = index[11] + s; // 下面是92次加法
        // 80次+=, 4次=
        eData[id0] = temp[id1];
        eData[id0] += temp[id2];
        eData[id0] += temp[id3];
...
#pragma omp critical
    ...
    std::cout << "encodeTo48Ctxt time: " << std::chrono::duration<double>(m2 - m1).count() << "s\n";
    ...
    std::cout << "encodeTo48Ctxt time: " << std::chrono::duration<double>(m4 - m3).count() << "s\n";
