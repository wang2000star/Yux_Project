                 ...
                 const EncryptedArrayDerived<PA_GF2> &ea)
{
    ...
    Vec<ZZX> encoded;
    encodeTo16Ctxt(encoded, Xset, ea); // encode as HE plaintext
    {
        Ctxt tmpCtxt(hePK);
                  ...
                  const EncryptedArrayDerived<PA_GF2> &ea)
{
    ...
    // 对expanded进行simd编码，这样会返回nRoundKeys个多项式数组即encoded，nRoundKeys=encoded.length()
    Vec<ZZX> encoded;
    encodeTo16Ctxt(encoded, expanded, ea); // encode as HE plaintext
...
bool Server_offline()
{
    ...
    if (!verifyDecryption16(encryptedXset, secretKey, ea, Xset))
    {
    ...
    if (!verifyDecryption16(encrypted_RoundKeySet, secretKey, ea, RoundKeySet))
    {
    ...
    if (!verifyDecryption16(encryptedIV, secretKey, ea, expandedIV))
    {
