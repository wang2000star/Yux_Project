...
// 函数：解密并验证密文是否正确
bool verifyDecryption(const vector<Ctxt>& encryptedVec, const SecKey& secretKey,
                 ...
                 const EncryptedArrayDerived<PA_GF2> &ea)
{
    ...
    if (IsZero(encoded[i])) {
        std::cerr << "Encoded polynomial at index " << i << " is zero!" << std::endl;
    } 
}
    {
        Ctxt tmpCtxt(hePK);
...
bool Server_offline() {
    ...
    if (!verifyDecryption(encryptedXset, secretKey, ea, Xset)) {
    ...
    if (!verifyDecryption(encrypted_RoundKeySet, secretKey, ea, RoundKeySet)) {
    ...
    if (!verifyDecryption(encryptedIV, secretKey, ea, expandedIV)) {
    ...
    auto start_KeyStream_FHE = std::chrono::steady_clock::now();

    vector<Ctxt> encryptedKeyStream;
    encryptedKeyStream.resize(encryptedIV.size(), tmpCtxt);
    //vector<Ctxt> encryptedKeyStream(encryptedIV.size(), Ctxt(ZeroCtxtLike, encryptedIV[0]));
    ...
    if (!verifyDecryption(encryptedKeyStream, secretKey, ea, KeyStream)) {
    ...
    auto end_KeyStream_FHE = std::chrono::steady_clock::now();
    std::chrono::duration<double> elapsed_seconds_KeyStream_FHE = end_KeyStream_FHE - start_KeyStream_FHE;

    std::cout << "KeyStream FHE succeeded! Total Time: " << elapsed_seconds_KeyStream_FHE.count() << "s\n";
...
bool Server_online()
{
    ...
    printf("CipherStream encryption succeeded!\n");
#endif
    ...
    encryptedPlainStream.resize(encryptedCipherStream.size(), Ctxt(ZeroCtxtLike, encryptedCipherStream[0]));
    ...
    printf("PlainStream computation succeeded!\n");
#endif
