...
int main()
{
    ...
    for (long r = 1; r < Nr; r++)
    {
        ...
        // Linear Layer
        HE_MC(encryptedKeyStream ,evaluator);
        HE_MR(encryptedKeyStream ,evaluator);
...
// 最后一轮
    ...
    {
        filePath = "test_HERA_4.txt";
    }
    else
    {
        filePath = "../tests/test_HERA_4.txt";
    }
    ...
    outfile << std::left << std::setw(3) << Nr
            ...
            << std::endl;
    outfile.close();
    std::cout << "test_HERA_4.txt updated." << std::endl;
    }
    return 0;
}